...
var empty,moves,completed=false;
function action_event(x)
      {
        if(completed)
            return;
                ...
                if((Math.abs(emptyI - selectedI) == 1 && emptyJ == selectedJ) || (Math.abs(emptyJ - selectedJ) == 1 && emptyI == selectedI)) 
                      {
                        ...
                        document.getElementById(empty).className = "slot";
                        document.getElementById(empty).innerHTML = x.innerHTML;
                        x.className = "slot empty";
                        x.innerHTML = '';
                        empty = id_selected;
                        numberofmoves++;
                        document.getElementById("moves").innerHTML = "Moves so far: " + numberofmoves;
                        if(finished())
                          {
                            complete = true;
                            document.getElementById("moves").innerHTML = "CONGRATS! Number of moves it took to complete: " + numberofmoves;
                          }
                      }
              }
      }
...
function startPuzzle() 
          {
            ...
            randomNumber = boardnumbers.sort(function () { return (Math.round(Math.random())-0.5);});
